...
#ifndef __FTP_SERVER_COMMAND_HEADER__
#define __FTP_SERVER_COMMAND_HEADER__
...
//If the client is not logged in, sends an appropriate response to the client and closes all
//connections by calling 'handleNotLoggedIn()' function.
//If the client is logged in, determines whether moving up to the parent directory is not falling beyond
...
//If the client is not logged in, sends an appropriate response to the client and closes all connections
//by calling 'handleNotLoggedIn()' function.
//If the client is logged in, it checks whether the data connection has already been established by
...
//If a data connection has already been established, prepares and sends the list of current
...
//Sends the count of the entries in an appropriate response message to the client using the control connection.
...
//If the client is not logged in, sends an appropriate response to the client and closes all
//connections by calling 'handleNotLoggedIn()' function.
//If the client is logged in, it checks whether the data connection has already been established by a PASV
//command before this RETR command.
//If a data connection has already been established 'dataSockDescriptor' parameter should have a non-negative value.
//If a data connection has not been established, sends an appropriate response to the user.
//If a data connection has already been established, reads from the file and sends the content of the file by
